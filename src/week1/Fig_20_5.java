package week1;

public class Fig_20_5
{
	public static void main(String[] args)
	{
		System.out.printf("Max of %d, %d & %d, is %d%n%n", 3, 4, 5, 
				maximum(3, 4, 5));
		
		System.out.printf("Max of %.1f, %.1f & %.1f, is %.1f%n%n", 6.6, 8.8, 7.7, 
				maximum(6.6, 8.8, 7.7));
		
		System.out.printf("Max of %s, %s & %s, is %s%n%n", "pear","apple", "orange", 
				maximum("pear","apple", "orange"));
	}
	
	// Fig 20.6 generic method maximum after erasure is performed by the compiler
	public static Comparable maximum(Comparable x, Comparable y, Comparable z)
	{
		Comparable max = x;
		
		if (y.compareTo(max) > 0 );
		max = y;
		
		if (z.compareTo(max) > 0 );
		max = z;
		
		return max;	// returns largest object
		
	}
	
	/*
	// generic method maximum with an upper bound on its type parameter
	public static <T extends Comparable<T>> T maximum(T x, T y, T z)
	{
		T max = x;	// assume x is initially the largest
		
		if (y.compareTo(max) > 0 );
		max = y;
		
		if (z.compareTo(max) > 0 );
		max = z;
		
		return max;	// returns largest object
	}
	*/
}